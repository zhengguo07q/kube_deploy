...
WORKDIR $GOPATH/dolphin/professional_server/app/third_party
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app-thirdparty .
...
#把上面生成的执行文件拷贝进bin
COPY --from=0 /app-thirdparty /usr/local/bin/app-thirdparty

#入口点执行
ENTRYPOINT ["/usr/local/bin/app-thirdparty"]
