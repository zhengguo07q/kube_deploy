...
WORKDIR $GOPATH/dolphin/professional_server/app/organization
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app-organization .
...
#把上面生成的执行文件拷贝进bin
COPY --from=0 /app-organization /usr/local/bin/app-organization

#入口点执行
ENTRYPOINT ["/usr/local/bin/app-organization"]
