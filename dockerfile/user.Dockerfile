...
WORKDIR $GOPATH/dolphin/professional_server/app/user
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app-user .
...
#把上面生成的执行文件拷贝进bin
COPY --from=0 /app-user /usr/local/bin/app-user

#入口点执行
ENTRYPOINT ["/usr/local/bin/app-user"]
