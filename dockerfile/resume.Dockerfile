...
WORKDIR $GOPATH/dolphin/professional_server/app/resume
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app-resume .
...
#把上面生成的执行文件拷贝进bin
COPY --from=0 /app-resume /usr/local/bin/app-resume

#入口点执行
ENTRYPOINT ["/usr/local/bin/app-resume"]
