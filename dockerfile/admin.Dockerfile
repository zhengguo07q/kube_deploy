...
WORKDIR $GOPATH/dolphin/professional_server/app/admin
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app-admin .
...
#把上面生成的执行文件拷贝进bin
COPY --from=0 /app-admin /usr/local/bin/app-admin

#入口点执行
ENTRYPOINT ["/usr/local/bin/app-admin"]