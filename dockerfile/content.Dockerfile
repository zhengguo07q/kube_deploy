...
WORKDIR $GOPATH/dolphin/professional_server/app/content
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o /app-content .
...
#把上面生成的执行文件拷贝进bin
COPY --from=0 /app-content /usr/local/bin/app-content

#入口点执行
ENTRYPOINT ["/usr/local/bin/app-content"]
